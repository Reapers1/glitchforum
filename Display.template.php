...
function template_main()
{
	...
	//Veo - MySQL Game Server Bağlantısı
    include('baglan.php');
	//---
	...
	echo '
			<div id="forumposts">
				<div class="cat_bar">
					<h3 class="catbg">
						<i class="fa fa-edit fa-2x" style="margin-top: 12px;"></i>
						<div class="veo-yazar">', $context['user']['name'], '</div>
						<div class="veo-konuadı">', $context['subject'], '</div>
						<div class="veo-konugörüntülenmesi"><i class="fa fa-eye"></i>  ', $context['num_views'], '</div>
					</h3>
				</div>';
	...
	while ($message = $context['get_message']())
	{
		...
		echo '
				...
				if (isset($message['member']['group_id']))  {
					...
					case '1':
					   $veobg = 'background-image: url('. $settings['images_url']. '/postbit/1.png);color: #fff;';//admin grubu için renk ayarları
						break;
					case '2':
					   $veobg = 'background-image: url('. $settings['images_url']. '/postbit/1.png);color: #fff;';//global mod için renk ayarları
						break;
				 case '0':
					   $veobg = 'background-image: url('. $settings['images_url']. '/postbit/oyuncu.png);color: #fff;';//normal üyeler için renk ayarları
						 break;
				 default:
				   $veobg = 'background-image: url('. $settings['images_url']. '/postbit/oyuncu.png);color: #fff;';//grub listesinde olmayanlar renk ayarları
				 break;
					 }
				 }
				 else {$veobg= 'background-image: url('. $settings['images_url']. '/postbit/1.png);color: #fff;';}
				 //Veritabanından kullanıcı bilgilerini postbit üzerine alıyoruz
				 $forumnick = $message['member']['name'];
					$karakterbul = mysql_num_rows(mysql_query("SELECT * FROM characters WHERE charactername = '$forumnick'"));
					if($karakterbul >= 1)
					{
						$k_bul = mysql_query("SELECT * FROM characters WHERE charactername = '$forumnick'");
						$ara2 = mysql_fetch_assoc($k_bul);
						$k_adi = $ara2['charactername'];
						$k_skin = $ara2['skin'];
						$k_level = $ara2['level'];
						/*
						$k_fara = mysql_query("SELECT * FROM factionlar WHERE FactionID = '$k_faction'");
						$k_fara2 = mysql_fetch_assoc($k_fara);
						if($k_faction > -1) {
							$k_olusum = $k_fara2['FactionAdi'];
						}
						else {
							$k_olusum = "Sivil";
						}
				 ...
				 echo '
				 <div class="veoroundframe" style="'.$veobg.'">
      ...
      echo '          <div class="left_colum"> <!--Inicio segunda columna-->
		...
		if (!$message['member']['is_guest'])
		{
			...
			echo '
								<div class="veo-rank">', $message['member']['group_stars'], '</div>';
			...
			echo'</div>
                  ...
                  <div class="veoinfop">
				<div class="veo-sosyal">Sosyal Ağlar</div>
			...
			if ($settings['show_profile_buttons'])
			{
				...
				if ($message['member']['can_view_profile'])
					echo '
										<li><a href="', $message['member']['href'], '">', ($settings['use_image_buttons'] ? '<i class="fa fa-user-circle fa-1x" aria-hidden="true" style="color:#fff;" alt="' . $txt['view_profile'] . '" title="' . $txt['view_profile'] . '"></i>' : $txt['view_profile']), '</a></li>';

				// Don't show an icon if they haven't specified a website.
				if ($message['member']['website']['url'] != '' && !isset($context['disabled_fields']['website']))
					echo '
										<li><a href="', $message['member']['website']['url'], '" title="' . $message['member']['website']['title'] . '" target="_blank" class="new_win">', ($settings['use_image_buttons'] ? '<i class="fa fa-globe fa-1x" style="color:#fff;" aria-hidden="true" alt="' . $message['member']['website']['title'] . '"></i>' : $txt['www']), '</a></li>';

				// Don't show the email address if they want it hidden.
				if (in_array($message['member']['show_email'], array('yes', 'yes_permission_override', 'no_through_forum')))
					echo '
										<li><a href="', $scripturl, '?action=emailuser;sa=email;msg=', $message['id'], '" rel="nofollow">', ($settings['use_image_buttons'] ? '<i class="fa fa-envelope fa-1x" aria-hidden="true" alt="' . $txt['email'] . '" title="' . $txt['email'] . '" style="color:#fff;"></i>' : $txt['email']), '</a></li>';

				// Since we know this person isn't a guest, you *can* message them.
				if ($context['can_send_pm'])
					echo '
										<li><a href="', $scripturl, '?action=pm;sa=send;u=', $message['member']['id'], '" title="', $message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline'], '">', $settings['use_image_buttons'] ? '<i class="fa fa-paper-plane fa-1x" style="color:#fff;"aria-hidden="true" alt="' . ($message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline']) . '"></i>' : ($message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline']), '</a></li>';
		...
		echo '
			...
			$message['member']['options']['teknoromi']='0';
			}
			echo '
						<div class="veo-fav-karakter">
			<img src="', $settings['default_images_url'], '/karakter/', $k_skin, '.png" /></div> ';
			echo'
				  
				<div class="veo-api-karakter"><i class="fa fa-user" aria-hidden="true" style="color: #fff"></i> ',$k_adi,'</div>
				<div class="veo-api-seviye">',$k_level,' level</div>
				</div>
		...
		if ($message['id'] == $context['first_message'] && $context['user']['is_logged'])
		echo '
		 <script>function fbs_click() {u=location.href;t=document.title;window.open(\'http://www.facebook.com/sharer.php?u=\'+encodeURIComponent(u)+\'&t=\'+encodeURIComponent(t),\'sharer\',\'toolbar=0,status=0,width=626,height=436\');return false;}</script><style> html .fb_share_button { font-size: 30px;
		 margin-left: 6px; color: #3b7398;  display: -moz-inline-block; display:inline-block; padding:1px 20px 0 5px; height:15px;) 
		 no-repeat top right; } html .fb_share_button:hover { color: #fff;  text-decoration:none; } </style> <a href="http://www.facebook.com/share.php?u=<url>" class="fb_share_button" onclick="return fbs_click()" target="_blank" alt="Facebook\'da Paylaş"style="text-decoration:none;"><i class="fa fa-facebook-official" aria-hidden="true"></i></a>';
		...
		if ($message['can_modify'])
			echo '
							<img src="', $settings['images_url'], '/icons/modify_inline.png" alt="', $txt['modify_msg'], '" title="', $txt['modify_msg'], '" class="modifybutton" id="modify_button_', $message['id'], '" style="cursor: ', ($context['browser']['is_ie5'] || $context['browser']['is_ie5.5'] ? 'hand' : 'pointer'), '; display: none;" onclick="oQuickModify.modifyMsg(\'', $message['id'], '\')" />';
		...
		if (!empty($message['attachment']))
		{
			...
			foreach ($message['attachment'] as $attachment)
			{
				...
				echo '
										<a href="' . $attachment['href'] . '" title="' . $attachment['name'] . '"><img src="' . $settings['images_url'] . '/indir.png" align="middle" alt="*" /></a> ';
			...
			<div class="tborder" id="quickreplybox">
				...
				<div id="quickReplyOptions"', $options['display_quick_reply'] == 2 ? '' : ' style="display: none"', '>
					...
					if (isset($message['member']['group_id']))  {
						switch ($message['member']['group_id'])       {
						case '1':
						   $veobg = 'background-color: #800000;color: #fff;';//admin grubu için renk ayarları
							break;
						case '2':
						   $veobg = 'background-color:blue;color:yellow;';//global mod için renk ayarları
							break;
					 case '0':
						   $veobg = 'background-color:orange;color:green;';//normal üyeler için renk ayarları
							 break;
					 default:
					   $veobg = '';//grub listesinde olmayanlar renk ayarları
					 break;
						 }
					 }
					 else {$veobg= '';}
